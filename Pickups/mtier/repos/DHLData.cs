...
namespace Pickups.mtier.repos
{
    public class DHLData
    {
        ...
        public List<DHLData> get()
        {
            List<DHLData> result = null;
            ...
            try
            {
                using (var c = Connectors.getConnection())
                {
                    try
                    {
                        var cd = new SqlCommand(@"select * from dbo.DHLCorrectData where process = @processName;", c);
        ...
        public bool isLoaded(string fn)
        {
            using (var c = Connectors.getConnection())
            {
                try
                {
                    var cd = new SqlCommand(@"select count(*) from dbo.DHLCorrectData where process = @p and isLoaded = @isL;", c);
        ...
        public bool PersistDHLData(string fn)
        {
            using (var c = Connectors.getConnection())
            {
                try
                {
                    var cd = new SqlCommand(@"update dbo.DHLCorrectData set isLoaded = @isLoaded where process = @process;", c);
            ...
            using (var c = Connectors.getConnection())
            {
                try
                {
                    using (var trans = c.BeginTransaction())
                    {
                        try
                        {
                            var cd = new SqlCommand(@"delete from dbo.DHLCorrectData where process = @process and isLoaded = @isLoadStatus;", c, trans);
                            cd.Parameters.AddWithValue("@process", fn);
        ...
        public List<DHLData> getDistinctCorrectDHLFiles()
        {
            List<DHLData> result = null;
            using (var c = Connectors.getConnection())
            {
                try
                {
                    var cd = new SqlCommand(@"select distinct process from dbo.DHLCorrectData where isLoaded = @isLoadStatus;", c);
        ...
        public bool getApprovingStatus(string proc)
        {
            ...
            using (var c = Connectors.getConnection())
            {
                try
                {
                    var cd = new SqlCommand(@"select top 1 [status] from dbo.DHLCorrectData where process = @process;", c);
                    cd.Parameters.AddWithValue("@process", proc);
        ...
        public bool getAuthorizationStatus(string proc)
        {
            //determines if a particular record has been authorized or otherwise
            using (var c = Connectors.getConnection())
            {
        ...
        public bool ApproveCorrectDHLData(string proc, int official_Id)
        {
            ...
            try
            {
                using (var c = Connectors.getConnection())
                {
                    try
                    {
                        var cd = new SqlCommand(@"update dbo.DHLCorrectData set status = @status, branchOfficerId = @officerId where process = @process;", c);
        ...
        public List<DHLData> getDistinctIASRecordsUsingFileStatus(int L, int S)
        {
            List<DHLData> r = null;
        ...
        public bool AuthorizeDHLData(string process, int official_Id)
        {
            ...
            try
            {
                using (var c = Connectors.getConnection())
                {
                    try
                    {
                        var cd = new SqlCommand(@"update dbo.DHLCorrectData set status = @status, HOfficerId = @officerId where process = @process;", c);
                        ...
                        cd.Parameters.AddWithValue("@process", process);
